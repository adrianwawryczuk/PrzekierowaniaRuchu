...
char* plikDoChar(const char* nazwa_pliku)
{
	...
	auto* pbuf = plik.rdbuf();
	streamoff wielkosc_pliku = pbuf->pubseekoff(0, ios::end, ios::in);
	plik.seekg(0);

	auto* wyjscie = new char[static_cast<int>(wielkosc_pliku) + 1];
...
void writeAllNodesFromOSM(xml_node<>* korzen)
{
	...
	auto* prep_stmt = MySqlConnection::getMySqlConnection().con->prepareStatement("Insert into node(id, lat, lon) values(?, ?, ?)");
...
void checkAllWays(xml_node<>* korzen)
{
	...
	auto all = 0, a = 0, b = 0, c = 0, d = 0, e = 0, f = 0, g = 0, h = 0, i = 0, j = 0, k = 0, l = 0, withoutTags = 0, m = 0, n = 0;
	auto o = 0, p = 0, r = 0, s = 0, t = 0;
	string restofnames = "", tempRestOfNames = "";
	auto foundTag = false, foundExpectedName = false;
...
void deleteUnusedNodes()
{
	...
	auto* statesment = MySqlConnection::getMySqlConnection().con->createStatement();
